...
왜 UDP를 사용할까? UDP는 상대방과 connenction을 확인하는 과정(hand shaking)을  거치지 않고 오로지 데이터 전송에만 집중한다. **TCP는 connection 수립하는 hand shaking과 connection을 통해 트래픽을 관리하는 오버헤드가 발생**한다. 즉, 메시지를 하나만 보내는 간단한 어플리케이션에서 TCP를 사용하는 것은 배보다 배꼽이 더 큰 것과 같다. 스트리밍과 같이 데이터의 손실보다 처리율이 중요한 경우에는 UDP를 사용한다.
...
![image-20201030174708098](image/CH2_애플리케이션계층/image-20201030174708098.png)
...
![image-20201030174812772](image/CH2_애플리케이션계층/image-20201030174812772.png)
...
![image-20201030180732571](image/CH2_애플리케이션계층/image-20201030180732571.png)
